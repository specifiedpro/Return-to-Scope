...
'''
#%%
def dea_dual(dmu:list, x:np.ndarray, y:np.ndarray,  THRESHOLD=0.000000000001, orient=const.INPUT_ORIENT, rs=const.VRS, lambda_k_type='CONTINUOUS'):
    ...
    for r in range(K):
        eff_dict[dmu[r]], r_lambdas_dict = solver_r_dual(dmu=dmu, r=r, x=x, y=y, THRESHOLD=THRESHOLD, rs=rs, lambda_k_type=lambda_k_type)
