...
'''
#%%
def io_dual(dmu:list, r:int, x:np.ndarray, y:np.ndarray,  THRESHOLD=0.000000000001, rs=const.VRS, lambda_k_type='CONTINUOUS'):
    ...
    if lambda_k_type == 'CONTINUOUS':
        for k in range(K):
            lambda_k[k]=m.addVar(vtype=gp.GRB.CONTINUOUS,name=f"lambda_k{k}", 
                lb=THRESHOLD
                )
    elif lambda_k_type == 'BINARY':
        for k in range(K):
            lambda_k[k]=m.addVar(vtype=gp.GRB.BINARY,name=f"lambda_k{k}", 
                lb=THRESHOLD
                )
    m.update()
...
#%%
def oo_dual(dmu:list, r:int, x:np.ndarray, y:np.ndarray,  THRESHOLD=0.000000000001, rs=const.VRS, lambda_k_type='CONTINUOUS'):
    ...
    # FDH: Change vtype from CONTINUOUS to BINARY
    if lambda_k_type == 'CONTINUOUS':
        for k in range(K):
            lambda_k[k]=m.addVar(vtype=gp.GRB.CONTINUOUS,name=f"lambda_k{k}", 
                lb=THRESHOLD
